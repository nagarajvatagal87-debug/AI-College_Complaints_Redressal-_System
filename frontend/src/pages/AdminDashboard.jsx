...
export default function AdminDashboard() {
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      const r = await fetch(`${API_URL}/api/admin/dashboard`);
  ...
  const fetchCategoryData = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_URL}/api/admin/complaints-by-category`
      );
      const d = await r.json();
  ...
  const fetchStatusData = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_URL}/api/admin/complaints-by-status`
      );
      const d = await r.json();
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const r = await fetch(`${API_URL}/api/admin/notifications`);
      const d = await r.json();
  ...
  const autoAssignStaff = async (complaintId) => {
    ...
    try {
      const r = await fetch(
        `${API_URL}/api/admin/complaints/${complaintId}/auto-assign`,
        {
          method: "POST",
        }
      );
      ...
      alert("Server error during auto-assign");
    }
  ...
  const filtered = complaints.filter((c) => {
    ...
    return (
      ...
      (!toDate ||
        new Date(c.created_at) <= new Date(`${toDate}T23:59:59`))
    );
  });
  ...
  const timeAgo = (d) => {
    const mins = Math.floor(
      (now.getTime() - new Date(d).getTime()) / 60000
    );
  ...
  const notifDot = (n) =>
    n.message?.includes("New") || n.message?.includes("🆕")
      ? "#ef4444"
      : n.message?.includes("Resolved")
      ? "#10b981"
      : n.message?.includes("assigned") || n.message?.includes("AI")
      ? "#6366f1"
      : "#f59e0b";
      ...
      {aiResult && (
        ...
        >
          <div style={{ fontWeight: 700, marginBottom: 6 }}>
            🤖 AI Auto-Assigned #{aiResult.id}
          </div>

          <div style={{ opacity: 0.9, marginBottom: 4 }}>
            ✅ Assigned to: <strong>{aiResult.assigned}</strong>
          </div>
      ...
      {staffModal && (
        ...
        >
          ...
          >
            ...
            >
              There's no staff member with role{" "}
              <strong>"{staffModal.suggestedRole}"</strong> yet.
              Create one below — they'll get an email with login details and
              be assigned this complaint immediately.
            </div>
            ...
            >
              ...
              <button
                onClick={createStaffAndAssign}
                disabled={staffSaving}
                style={{
                  padding: "9px 18px",
                  borderRadius: 9,
                  border: "none",
                  background: staffSaving
                    ? "#a5b4fc"
                          ...
                          {item.assigned_to ? (
                            ...
                            <button
                              ...
                              style={{
                                ...
                                alignItems: "center",
                                gap: 6,
                                background: aiAssigning[
                                  item.id
                                ]
                                  ? "#e5e7eb"
                                  : "linear-gradient(135deg,#6366f1,#8b5cf6)",
                                color: aiAssigning[item.id]
                                  ? "#9ca3af"
                                  : "#fff",
                                border: "none",
                                padding: "8px 14px",
                                ...
                                fontSize: 12,
                                fontWeight: 600,
                                cursor: aiAssigning[item.id]
                                  ? "not-allowed"
                                  : "pointer",
                                boxShadow: aiAssigning[
                                  item.id
                                ]
                                  ? "none"
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <button
                            ...
                            style={{
                              ...
                              cursor: "pointer",
                              boxShadow:
                                "0 2px 7px rgba(59,130,246,0.35)",
                            }}
                          >
                            👁 View
                          </button>
