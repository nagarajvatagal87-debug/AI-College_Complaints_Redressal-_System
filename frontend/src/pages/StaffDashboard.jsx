    ...
    >
      <img
        src={`https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(
          value
        )}&bgcolor=ffffff&color=000000&margin=4`}
        alt="QR Code"
        width={size - 20}
        height={size - 20}
...
function ComplaintModal({ complaint, onClose }) {
  ...
  const handlePrintQR = () => {
    ...
    w.document.write(`
      <html>
        <head>
          <title>QR - Complaint #${complaint.id}</title>
        </head>
...
export default function StaffDashboard() {
  ...
  const fetchComplaints = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_URL}/api/staff/assigned-complaints`,
        {
          headers: {
      ...
      if (!r.ok) {
        throw new Error(
          `Failed to fetch complaints: ${r.status}`
        );
      }
      ...
      console.error(
        "Failed to fetch staff complaints:",
        e
      );
    }
  }, [getToken]);
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const r = await fetch(
        `${API_URL}/api/staff/notifications`,
        {
          headers: {
            Authorization: `Bearer ${getToken()}`,
          },
      ...
      if (!r.ok) {
        throw new Error(
          `Failed to fetch notifications: ${r.status}`
        );
      }
      ...
      console.error(
        "Failed to fetch staff notifications:",
        e
      );
    }
  }, [getToken]);
  ...
  const updateStatus = async (id, status) => {
    ...
    try {
      ...
      if (!r.ok) {
        throw new Error(
          `Failed to update complaint: ${r.status}`
        );
      }
      ...
      if (d.success) {
        ...
        alert(d.message || "Failed to update complaint.");
      }
    } catch (e) {
  ...
  const activeList = complaints.filter(
    (c) => c.status !== "Resolved"
  );
    ...
    <div className="sf-root">
      ...
      <header className="sf-topbar">
        ...
        <div className="sf-topbar-l">
          ...
          <div>
            <div className="sf-top-title">
              AI College Complaints Redressal System
            </div>

            <div className="sf-top-sub">
              Staff Dashboard
            </div>
          </div>
              ...
              <div className="sf-notif-drop">
                ...
                {notifications.length > 0 ? (
                  notifications.map((n, i) => (
                    <div
                      key={n.id || i}
                      className="sf-notif-item"
                    >
                      <span className="sf-notif-dot"></span>

                      <div className="sf-notif-txt">
                        {n.message}
                      </div>
                    </div>
                  ))
      ...
      <div className="sf-body">
        ...
        <div className="sf-welcome">
          ...
          <div className="sf-welcome-left">
            ...
            <div className="sf-welcome-hey">
              👋 {greeting()},
            </div>

            <div className="sf-welcome-name">
              {staff?.name || "Staff Member"}
            </div>

            <div className="sf-welcome-sub">
              Add remark, update status, then click
              Save. View image & QR via 👁 button.
            </div>
          ...
          <div className="sf-welcome-right">
            ...
            <div className="sf-role-circle">
              🔧
            </div>

            <div className="sf-role-label">
          ...
          {[
            ...
            >
              ...
              <div>
                ...
                <div className="sf-stat-lbl">
                  {c.label}
                </div>

                <div
                  className="sf-stat-val"
                  style={{
                    color: c.color,
                  }}
                >
